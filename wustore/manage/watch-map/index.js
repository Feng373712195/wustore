...
class watchMap {
    ...
    setWatchPageMap(id, watchs, isComponents, componentdata) {
        ...
        Object.keys(watchs).forEach(dataPath => {
            if (watchs[dataPath].deep) Object.assign(deepWatchs, {
                [dataPath]: watchs[dataPath].handle
            })
            else Object.assign(dataPathWatchs, {
                [dataPath]: watchs[dataPath]
            })
        })
    ...
    checkIsWatch(webviewId, dataPath, that, oldVal, newVal) {
        ...
        if (this.checkHasWatch(webviewId)) {
            ...
            if (watchs[dataPath]) {
                isFunction(watchs[dataPath]) && this.pushTodoWatchHandles(webviewId, watchs[dataPath].bind(that, oldVal, newVal));
            }
        }
        ...
        if (this.componentWatchs.get(webviewId) && this.componentWatchs.get(webviewId).get(componentId)) {
            ...
            if (dataPathWatchs[dataPath]) {
                isFunction(dataPathWatchs[dataPath]) && this.pushTodoWatchHandles(
                    componentId,
                    dataPathWatchs[dataPath].bind(that, oldVal, newVal),
                    true,
                    componentWatch);
            }
        }
    ...
    checkIsDeepWatch(nodeId, dataPath, that, oldVal, newVal, isComponents, componentWatch) {
        ...
        for (let i = 0; i < deepWatchKeys.length; i++) {
            if (dataPath.indexOf(deepWatchKeys[i]) === 0) {
                ...
                isComponents ?
                    this.pushTodoWatchHandles(nodeId, deepWatchs[deepWatchKeys[i]].bind(that, oldVal, newVal), true, componentWatch) :
                    this.pushTodoWatchHandles(nodeId, deepWatchs[deepWatchKeys[i]].bind(that, oldVal, newVal))
                isWatch = true
                break;
            }
        }
